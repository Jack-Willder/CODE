...
def student_profile_extractor(rollno):
    global attendance_profile_variable, head_result, data_result, max_array_length, remaining_head_list, remaining_data_list, onetime_head_runner_variable, remaining_data_length, viewstate, eventvalidation, test_total_days, data_total_days
    headers = {
    ...
    """*  GETTING THE DATE OUT OF 90 DAYS  *"""

    if test_total_days == 0:
        data_total_days = filtered_soupdata_03
        test_total_days += 1
...
def final_image_processor():
    global final_rollno, final_name, final_absent, final_od, final_percent, data_total_days
    final_rank = """01
02
03
04
...
46
"""
    temp_data_total_days_array = []
    for temp_data_total_days in data_total_days:
        temp_data_total_days_array.append(temp_data_total_days.text)
    data_total_days = temp_data_total_days_array[5]
    ...
    d.text((500 - 24, 2822), text=data_total_days, fill=(147, 147, 147), font=font, spacing=15)
...
#    print(r.status_code)
    ...
    viewstate = viewstate[0].get('value')
    eventvalidation = eventvalidation[0].get('value')
...
end_time = time.time()
print("\n\nTime Taken - {} seconds".format(int(end_time - start_time)))
