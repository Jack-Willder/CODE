...
def remaining_gap_filler(gap_length, gap_data_before):
    ...
    for remaining_head in remaining_head_list:
        data_result01.clear()
        head_result = head_result + remaining_head + "    "
        adder_length_of_eachremaininghead.append(len(head_result))
        temp_count_number_01 = 0

        for temp_data_result in data_result:
            temp_count_number_02 = 0
            if len(temp_data_result) < adder_length_of_eachremaininghead[temp_count_number_02]:
                temp_gap_length = int(adder_length_of_eachremaininghead[temp_count_number_02] - len(temp_data_result))
